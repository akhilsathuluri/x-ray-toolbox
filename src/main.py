...
class XRayTool:
    def __init__(self):
        ...
        with problem_config:
            ...
            self.selected_problem = problem_config.selectbox('Select problem', problem_list)
        ...
        ######################## Load problem class ################################
        ############################################################################
        mod = import_module('problems.'+self.selected_problem+'.library.'+self.selected_problem)
        ...
        with plot_config:
            ...
            self.qoi_alpha = plot_config.slider('QoI opacity', min_value = 0.0, max_value = 1.0, value = 1.0, step = 0.15)
            # Select the number of plots in a given row
            self.plot_cols = int(plot_config.number_input('Columns',min_value=3, step=1))
        ...
        with st.sidebar.form("dv_form"):
            ...
            with dv_expander:
                ...
                for i in range(self.problem_dv_size):
                    slider_step = (self.problem_dv.Upper[i]-self.problem_dv.Lower[i])/self.step_disc
                    slider_steps.append(slider_step)
                    (slider_dv.loc[i, 'Lower'], slider_dv.loc[i, 'Upper']) = dv_expander.slider(self.problem_dv.Variables[i], min_value = self.problem_dv.Lower[i].item(), max_value = self.problem_dv.Upper[i].item(), \
                    value = (float(slider_dv.Lower[i].item()), float(slider_dv.Upper[i].item())), step = slider_step, key = 'dv_slider'+str(i))

            qoi_expander = variable_container.expander('Quantities of Interest')
            with qoi_expander:
                # Make sliders available in the sidebar
                for i in range(self.problem_qoi_size):
                    (slider_qoi.loc[i, 'Lower'], slider_qoi.loc[i, 'Upper']) = qoi_expander.slider(self.problem_qoi.Variables[i], min_value = self.problem_qoi.Lower[i].item(), max_value = self.problem_qoi.Upper[i].item(), \
                    value = (float(slider_qoi.Lower[i].item()), float(slider_qoi.Upper[i].item())), step = (self.problem_qoi.Upper[i]-self.problem_qoi.Lower[i])/self.step_disc, key = 'qoi_slider'+str(i))
    ...
    def update_rectangles(self, dv, fig):
        ...
        for plot_index in range(len(self.plotter)):
            if len(self.plotter[plot_index])==2:
                fig[plot_index].add_shape(type='rect',\
                x0=dv.Lower[self.plotter[plot_index][0]], y0=dv.Lower[self.plotter[plot_index][1]],\
                x1=dv.Upper[self.plotter[plot_index][0]], y1=dv.Upper[self.plotter[plot_index][1]], \
                ...
                if self.collapse_space == 'Nominal' or dv.Lower[self.plotter[plot_index][1]] == dv.Upper[self.plotter[plot_index][1]] and \
                    ...
                    fig[plot_index].add_shape(type='line', \
                    x0=self.problem_dv.Lower[self.plotter[plot_index][0]], y0=dv.Lower[self.plotter[plot_index][1]], \
                    x1=self.problem_dv.Upper[self.plotter[plot_index][0]], y1=dv.Upper[self.plotter[plot_index][1]], \
                    line=dict(color="Black", width=4, dash='dot'),
                    )

                    fig[plot_index].add_shape(type='line', \
                    x0=dv.Lower[self.plotter[plot_index][0]], y0=self.problem_dv.Lower[self.plotter[plot_index][1]], \
                    x1=dv.Upper[self.plotter[plot_index][0]], y1=self.problem_dv.Upper[self.plotter[plot_index][1]], \
                    line=dict(color="Black", width=4, dash='dot'),
                    )

            if len(self.plotter[plot_index])==3:
                # Define the verticies of a cube mesh
                xrange = [dv.Lower[self.plotter[plot_index][0]], dv.Upper[self.plotter[plot_index][0]]]
                yrange = [dv.Lower[self.plotter[plot_index][1]], dv.Upper[self.plotter[plot_index][2]]]
                zrange = [dv.Lower[self.plotter[plot_index][2]], dv.Upper[self.plotter[plot_index][2]]]
                xx = [xrange[0]]*4+[xrange[1]]*4
                yy = [yrange[0]]*2+[yrange[1]]*2+[yrange[0]]*2+[yrange[1]]*2
                zz = [zrange[0], zrange[1], zrange[0], zrange[1], zrange[0], zrange[1], zrange[0], zrange[1]]

                fig[plot_index].add_trace(go.Mesh3d(x=xx,y=yy,z=zz, \
                ...
                if self.label_dropdown:
                    with col[plot_col].expander(''):
                        st.write('''
                            {} : {} \n
                            {} : {}
                        '''.format(self.problem_dv.Variables[self.plotter[plot_index][0]], self.problem_dv.Description[self.plotter[plot_index][0]], self.problem_dv.Variables[self.plotter[plot_index][1]], self.problem_dv.Description[self.plotter[plot_index][1]]))
    ...
    def evaluate(self, dv_samples, plot_index):
        ...
        for i in range(len(self.prob.eval_list)):
            masks[self.prob.eval_list[i]] = (self.prob.var[self.prob.eval_list[i]] < float(st.session_state.updated_qoi.Upper.loc[st.session_state.updated_qoi.Variables==self.prob.eval_list[i]]))&(self.prob.var[self.prob.eval_list[i]] > float(st.session_state.updated_qoi.Lower.loc[st.session_state.updated_qoi.Variables==self.prob.eval_list[i]]))
    ...
    def compute_mappings(self):
        ...
        for plot_index in range(len(plotter)):
            ...
            if (session_change == slider_change) or sample_size_change or self.rerun_button:
                ...
                self.prob.var.to_csv(self.problem_path+'/output/plot_data_'+str(plot_index)+'.csv', index=False)
            else:
                # Load the saved data
    ...
    def scatter_plots(self, fig):
        ...
        for plot in range(len(self.plotter)):
            if len(self.plotter[plot_index])==2:
                trace_ok = go.Scatter(x = self.dv_samples_ok[plot_index].iloc[:, self.plotter[plot_index][0]], \
                                      y = self.dv_samples_ok[plot_index].iloc[:, self.plotter[plot_index][1]], mode= 'markers', name = 'feasible designs', marker=self.scatter_markers_ok)
                ...
                for idx in self.masks[plot_index].columns:
                    fig[plot_index].add_trace(go.Scatter(x = self.dv_samples_per_plot[plot_index][~self.masks[plot_index][idx]].iloc[:, self.plotter[plot_index][0]], \
                                                         y = self.dv_samples_per_plot[plot_index][~self.masks[plot_index][idx]].iloc[:, self.plotter[plot_index][1]],\
                                                         mode='markers', name=idx, marker=dict(size=self.marker_size, color='rgba({},{},{},{})'.format(st.session_state.cv[i,0], st.session_state.cv[i,1], st.session_state.cv[i,2], self.qoi_alpha))))
                    i = i+1

            if len(self.plotter[plot_index])==3:
                trace_ok = go.Scatter3d(x = self.dv_samples_ok[plot_index].iloc[:, self.plotter[plot_index][0]], \
                                      y = self.dv_samples_ok[plot_index].iloc[:, self.plotter[plot_index][1]], \
                                      z = self.dv_samples_ok[plot_index].iloc[:, self.plotter[plot_index][2]], mode= 'markers', name = 'feasible designs', marker=self.scatter_markers_ok)

                fig[plot_index].add_trace(trace_ok)
                # Isolating the designs NOT satisfying the constraints
                i = 0
                for idx in self.masks[plot_index].columns:
                    fig[plot_index].add_trace(go.Scatter3d(x = self.dv_samples_per_plot[plot_index][~self.masks[plot_index][idx]].iloc[:, self.plotter[plot_index][0]], \
                                                         y = self.dv_samples_per_plot[plot_index][~self.masks[plot_index][idx]].iloc[:, self.plotter[plot_index][1]],\
                                                         z = self.dv_samples_per_plot[plot_index][~self.masks[plot_index][idx]].iloc[:, self.plotter[plot_index][2]],\
                                                         mode='markers', marker=dict(size=self.marker_size, color='rgba({},{},{},{})'.format(st.session_state.cv[i,0], st.session_state.cv[i,1], st.session_state.cv[i,2], self.qoi_alpha))))
    ...
    def export_options(self):
        ...
        with export:
            if export.button('Save solution space'):
                ...
                st.session_state.updated_qoi.to_csv(output_path+'qoi_solution_space.csv', index=False)
